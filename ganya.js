...
function judge(page) {
  ...
  var config = {
    method: 'post',
    url: 'http://atcc-workshop.autopaddle.com/java/testProGanyacs/selectAllMagic',
    headers: {
  ...
  return axios(config)
    .then(function (response) {
      ...
      var judgeConfig = {
        method: 'post',
        url: 'http://atcc-workshop.autopaddle.com/java/testProGanyacs/judgePro',
        headers: {
          'User-Agent': 'apifox/1.0.0 (https://www.apifox.cn)',
...
const arr = Array.from({ length: 30 }).map((it, index) => index + 1)
